      ...
      <header className="bg-white/95 backdrop-blur-lg border-b border-zinc-100 sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-5 lg:px-8 h-14 flex items-center justify-between gap-2">
          <Link href="/" className="flex items-center gap-2 min-w-0">
            {brand.logo ? <img src={brand.logo} alt={brand.name} className="w-8 h-8 object-contain shrink-0" /> : (
              <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: bc }}><Home className="w-3.5 h-3.5 text-white" /></div>
            )}
            <span className="text-[14px] font-bold text-zinc-900 truncate">{locale === "ar" ? brand.name : brand.nameEn}</span>
          </Link>
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button onClick={() => setLocale(locale === "ar" ? "en" : "ar")} className="text-[12px] sm:text-[13px] text-zinc-400 hover:text-zinc-600 font-medium px-1.5 sm:px-2 py-1">{txt[locale].switchLang}</button>
            {isLoggedIn ? (
              <Link href="/dashboard" className="px-3 sm:px-3.5 py-1.5 sm:py-2 text-white text-[12px] sm:text-[13px] font-semibold rounded-xl hover:opacity-90 transition-colors whitespace-nowrap" style={{ backgroundColor: bc }}>
                <span className="hidden sm:inline">{txt[locale].backToDashboard}</span>
                <span className="sm:hidden">{locale === "ar" ? "التحكم" : "Dash"}</span>
              </Link>
            ) : (
              <Link href="/login" className="px-3 sm:px-3.5 py-1.5 sm:py-2 text-white text-[12px] sm:text-[13px] font-semibold rounded-xl hover:opacity-90 flex items-center gap-1.5 transition-colors whitespace-nowrap" style={{ backgroundColor: bc }}><LogIn className="w-3.5 h-3.5" /><span className="hidden sm:inline">{txt[locale].login}</span></Link>
            )}
          </div>
        </div>
      </header>
